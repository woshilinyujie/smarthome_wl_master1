package com.fbee.smarthome_wl.widget.pop;
...
public class PopwindowChoose extends PopupWindow{
    ...
    public PopwindowChoose(Context context, List<Menu> menus, BaseRecylerAdapter.OnItemClickLitener listener, View.OnClickListener clickListener, boolean isshow) {
        super(context);
        this.isshow = isshow;
         ...
         * 初始化View与监听器
         */
        initView(mContentView,menus,listener,clickListener);
    }

    private TextView tvAdd;
    private void initView(View mContentView, List<Menu> menus,BaseRecylerAdapter.OnItemClickLitener listener,View.OnClickListener clickListener) {
        RecyclerView  rvMenuList = (RecyclerView) mContentView.findViewById(R.id.rv_menu_list);
        tvAdd = (TextView) mContentView.findViewById(R.id.tv_add);
        if(isshow){
