...
import com.fbee.smarthome_wl.event.JiuCaiMsgEvent;
import com.fbee.smarthome_wl.event.JiuCaiNetPicEvent;
 ...
 */
public abstract class BaseActivity<T extends BasePresenter> extends AppCompatActivity implements View.OnClickListener, ICVSSListener, AnyChatTransDataEvent, AnyChatUserInfoEvent {
    /**
    ...
    ResAnychatLogin.UserBean.UserDevicesBean devicesBean;
...
//        }
    ...
    public AnyChatBaseEvent anyChatBaseEvent = new AnyChatBaseEvent() {
        @Override
        public void OnAnyChatConnectMessage(boolean bSuccess) {
            if (!bSuccess) {
                /*if (isHome) {
                    ToastUtils.showShort(BaseActivity.this, getString(R.string.connet_vedio_fail));
					isHome=false;
				} else {
				}*/
                //                ToastUtils.showShort(BaseActivity.this,"connect anychat failure");
            } else {
        ...
        public void OnAnyChatLoginMessage(int dwUserId, int dwErrorCode) {
            if (dwErrorCode == 0) {
                //skipActivity(BaseActivity.this, MainActivity.class);
                //TODO 保存自己的id
            } else {
                //ToastUtils.showShort(BaseActivity.this, "login anychat failure");
            }
        ...
        public void OnAnyChatLinkCloseMessage(int dwErrorCode) {
            if (dwErrorCode != 0) {
                //ToastUtils.showShort(BaseActivity.this, getString(R.string.connet_fail) + dwErrorCode);
            } else {
                //statusLineCallback();
            }
        }
    ...
    public void onMeaasgeResponse(JSONObject jsonObject) {
        ...
        if (Call.equals(Method.METHOD_BDYLIST)) {
            ...
            boolean aBoolean = PreferencesUtils.getBoolean(from);
            if (aBoolean) {
    ...
    public void OnAnyChatTransFile(int dwUserid, String FileName, String TempFilePath, int dwFileLength, int wParam, int lParam, int dwTaskId) {
        ...
        LogUtil.e("OnAnyChatTransFile", "========" + FileName);
        LogUtil.e("OnAnyChatTransFile", "========" + TempFilePath);
        AnyChatOutParam anyChatOutParam = new AnyChatOutParam();
        anychat.QueryTransTaskInfo(dwUserid, dwTaskId, AnyChatDefine.BRAC_TRANSTASK_PROGRESS, anyChatOutParam);
        PreferencesUtils.saveList(gw.getUsername(), TempFilePath);
        RxBus.getInstance().post(new JiuCaiAlarmImage(TempFilePath));
//                ResAnychatLogin.UserBean.UserDevicesBean devicesBean = null;
//                List<ResAnychatLogin.UserBean.UserDevicesBean> listBean = AppContext.getDevices();
//                for (ResAnychatLogin.UserBean.UserDevicesBean bean : listBean) {
//                    if (dwUserid == Integer.valueOf(bean.getDevice_anychat_id())) {
//                        devicesBean = bean;
//                        break;
//                    }
//                }
//
//                Intent it = new Intent(this, EquesVideoCallActivity.class);
...
//        }
    ...
    public void OnAnyChatTransBuffer(int dwUserid, byte[] lpBuf, int dwLen) {
        String topActivity = AppUtil.getTopActivity(this);
        LogUtil.e("OnAnyChatTransBuffer+", topActivity + "+++" + new String(lpBuf));
        String buf = new String(lpBuf);

        try {
            if ("success".equals(buf)) {

            } else if ("action:doorbell".equals(buf)) {
                List<ResAnychatLogin.UserBean.UserDevicesBean> listBean = AppContext.getDevices();
                for (ResAnychatLogin.UserBean.UserDevicesBean bean : listBean) {
                    if (dwUserid == Integer.valueOf(bean.getDevice_anychat_id())) {
                        devicesBean = bean;
                ...
                startActivity(it);
                //请求访客图片
                anychat.TransBuffer(dwUserid, "action:imageRequest".getBytes(), 1024);
            } else if (buf.contains("LastedPicsNames")) {
                RxBus.getInstance().post(new JiuCaiMsgEvent(buf));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
