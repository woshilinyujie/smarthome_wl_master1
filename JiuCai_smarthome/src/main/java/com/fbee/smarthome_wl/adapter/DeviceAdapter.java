...
public class DeviceAdapter extends BaseAdapter {
    ...
    private final List<ResAnychatLogin.UserBean.UserDevicesBean> jiucaiDevices;
    private EquesListInfo.bdylistEntity bdylistEntity;
    private String onLinesBid;
    private String bid;

    public DeviceAdapter(Context context, List<EquesListInfo.bdylistEntity> bdylist, List<DeviceInfo> deviceInfos, List<ResAnychatLogin.UserBean.UserDevicesBean> jiucaiDevices) {
        this.jiucaiDevices = jiucaiDevices;
    ...
    @Override
    public int getCount() {
        return deviceInfos.size() + bdylist.size() + jiucaiDevices.size();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        } else if (jiucaiDevices.size() > position - (deviceInfos.size() + bdylist.size()) && position - (deviceInfos.size() + bdylist.size()) >= 0 && jiucaiDevices.size() > 0) {
            String device_name = jiucaiDevices.get(position - (deviceInfos.size() + bdylist.size())).getDevice_name();
            String alias = jiucaiDevices.get(position - (deviceInfos.size() + bdylist.size())).getAlias();
            if (alias != null && !alias.isEmpty()) {
                holder.name.setText(alias);
            } else {
                holder.name.setText(device_name);
            }
            holder.iv_icon_scenario.setImageResource(R.mipmap.eques_monitor);
        }
        return convertView;
