...
public class PreferencesUtils {
     ...
     * 存储九彩list集合
     */
    public static void saveList(String key, String jiucaiPic) {
        ArrayList<String> jiucaiVisitorPic = AppContext.getJiucaiVisitorPic();
        jiucaiVisitorPic.add(jiucaiPic);
        SharedPreferences.Editor editor = BaseApplication.getInstance().getContext().getSharedPreferences(SP_SETTING, 0).edit();
        editor.putInt(key, jiucaiVisitorPic.size());
        for (int i = 0; i < jiucaiVisitorPic.size(); i++) {
            editor.putString(key + i, jiucaiVisitorPic.get(i));
        }
        editor.commit();
    }

    public static ArrayList getList(String key) {
        ArrayList<String> JiucaiList = new ArrayList<>();
        SharedPreferences sp = BaseApplication.getInstance().getContext().getSharedPreferences(SP_SETTING, 0);
        int environNums = sp.getInt(key, 0);
        for (int i = 0; i < environNums; i++) {
            String environItem = sp.getString(key + i, null);
            JiucaiList.add(environItem);
        }
        return JiucaiList;
    }
