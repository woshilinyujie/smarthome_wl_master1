                ...
                if(originallyDevices.size()==0&&deleteDevicesInfo.size()>0){
                    ...
                    for (int i = 0; i < originallyDevices.size(); i++) {
                        ...
                        for (int j = 0; j <deleteDevicesInfo.size() ; j++) {
                            if(deleteDevicesInfo.get(j).getUId()==originallyDevices.get(i).getUId()){
                                flag = false;
                                break;
                            }
                            flag= true;
//                            needDeleteDevicesInfo.add(originallyDevices.get(i));
                        ...
                        if(flag){
                           int ret=serial.deleteDeviceFromGroup(originallyDevices.get(i),groupInfo);
                            try {
                                Thread.sleep(50);
                    ...
                    for (int i = 0; i <deleteDevicesInfo.size() ; i++) {
                        boolean  flag = false;
                        for (int j = 0; j <originallyDevices.size() ; j++) {
                            if(originallyDevices.get(j).getUId()==deleteDevicesInfo.get(i).getUId()){
                                flag = false;
                                break;
                            }
                            flag = true;
//                            needAddDevicesInfo.add(deleteDevicesInfo.get(i));
                        ...
                        if(flag){
                            ...
                            int ret = serial.addDeviceToGroup(deleteDevicesInfo.get(i),groupInfo);
                            try {
                                Thread.sleep(50);
                            } catch (InterruptedException e) {
                                e.printStackTrace();
