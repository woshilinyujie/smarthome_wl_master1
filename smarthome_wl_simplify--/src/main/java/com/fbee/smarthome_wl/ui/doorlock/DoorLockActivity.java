...
public class DoorLockActivity extends BaseActivity<DoorLockContract.Presenter> implements DoorLockContract.View {
    ...
    private void handleReceiveDoorLockState(DoorLockStateInfo event) {
        ...
        } else if (Flag == 2 || Flag == 0x7f || Flag == 0x13) {
            text.append("远程开锁未允许");
            mTextLockHint.setText(text.toString());
            ToastUtils.showShort("远程开锁未允许!");
            setReset(3000, false);
        }else if(Flag == 3){
            text.append("双人模式需要两个密码");
            mTextLockHint.setText(text.toString());
            ToastUtils.showShort("双人模式需要两个密码!");
            setReset(3000, false);
        }else if(Flag == 0x14){
            text.append("双人模式密码重复");
            mTextLockHint.setText(text.toString());
            ToastUtils.showShort("双人模式密码重复!");
            setReset(3000, false);
        }else if(Flag == 0x08){
            text.append("密码错误次数过多已禁用");
            mTextLockHint.setText(text.toString());
            ToastUtils.showShort("密码错误次数过多已禁用!");
            setReset(3000, false);
        }else if(Flag == 0x0A){
            text.append("临时密码时段受限");
            mTextLockHint.setText(text.toString());
            ToastUtils.showShort("临时密码时段受限!");
            setReset(3000, false);
        }
    ...
    private void setDoorLockState(final int state) {
        ...
        final ImageView swIv = (ImageView)dialog.findViewById(R.id.btn_close);
        swIv.setImageResource(R.mipmap.icon_single);
        swIv.setTag(R.mipmap.icon_single);
        swIv.setOnClickListener(
                new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        int src = (int)swIv.getTag();
                        if(src==R.mipmap.icon_single){
                            swIv.setImageResource(R.mipmap.icon_double);
                            swIv.setTag(R.mipmap.icon_double);
                            showToast("切换到双人模式");
                        }else{
                            swIv.setImageResource(R.mipmap.icon_single);
                            swIv.setTag(R.mipmap.icon_single);
                    ...
                    public void onClick(View v) {
                        ...
                        if (password.length() == 6) {
                            ...
                            //双人
                                ...
                                if(fristPassword ==null){
                                    fristPassword = et_password.getText().toString();
                                    showToast("请输入第二个密码");
                                    et_password.setText("");
                                    return;
                                }else{
                                    String secondPsw = et_password.getText().toString();
                                    final String doublepsw= fristPassword+secondPsw;
                                            ...
                                            if (gateWayInfo != null) {
                                                ...
                                                if (ret >=0) {
                                                    try {
                                                        if(res>=0){
                                                            AppContext.getInstance().getSerialInstance().setGatedoorStateID2(deviceInfo,
                                                                    state,   doubleencrypt(doublepsw.getBytes()));
                                                        }else{
                                                            byte[] aesByte = daes256encrypt(doublepsw.getBytes());
                                                            AppContext.getInstance().getSerialInstance().setGatedoorStateAES(deviceInfo,
                                                                    state, 6, aesByte);// 发送密文on
                                                        }
                                                    ...
                                                    AppContext.getInstance().getSerialInstance().setGatedoorState(deviceInfo,
                                                            state,  doubleencrypt(doublepsw.getBytes()));// 发送密文
                                                }
                                            }
     ...
     * 双人
     */
    public byte[] daes256encrypt(byte[] password) {
        ...
        byte[] key = new byte[32];
        byte key1[] = {0x46, 0x45, 0x49, 0x42, 0x49, 0x47,0x46, 0x45, 0x49, 0x42, 0x49, 0x47};// 加密密钥
        byte newPaString[] = new byte[12];// 异或后的密码
        //  password[i]&0x0f  此处密码需要先转化，再和密钥异或
        for (int i = 0; i < newPaString.length; i++) {
            newPaString[i] = (byte) (key1[i] ^ (password[i] & 0x0f));
        }
        ...
        byte[] doorPsd = new byte[32];
        //拼接加密前明文
        System.arraycopy(newPaString, 0, doorPsd, 0, 12);
        System.arraycopy(times, 0, doorPsd, 28, 4);
        byte[] aesPsd = null;
        try {
            //AES 256加密，具体操作看加密文件夹，需要替换jdk下的包
