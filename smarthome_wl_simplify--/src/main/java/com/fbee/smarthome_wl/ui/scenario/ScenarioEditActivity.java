 ...
 * @time 2017/3/27 9:56
 */
public class ScenarioEditActivity extends BaseActivity implements ScenariioEditAdapter.OnSwitchListener,ScenariioEditAdapter.OnStopSeekBarTouch ,ScenariioEditAdapter.OnDelayListener{
...
//    private List<DeviceInfo>  list;
    ...
    protected void initView() {
        ...
        tvDelaytime = (TextView) findViewById(R.id.tv_delaytime);
        ...
        layoutDelaytime = (RelativeLayout) findViewById(R.id.layout_delaytime);
    ...
    protected void initData() {
        ...
        if(getIntent().hasExtra("SenceBean")){
            ...
            if(senbean.getSenceDatas()!=null){
                list.addAll(senbean.getSenceDatas());
                tempList.addAll(senbean.getSenceDatas());
            }
            name=senbean.getSenceName();

            for (int i = 0; i <list.size() ; i++) {
               if(list.get(i).getuId() == 16449521) {

                   sencedata = list.get(i);
                   switch1.setChecked(true);
                   if(list.get(i).getData1()==1){
                       tvInstructions.setText("布防");
                   }else if(list.get(i).getData1()==2){
                       tvInstructions.setText("撤防");
                   }else{
                       tvInstructions.setText("");
                   }

                   if(list.get(i).getDelaytime() >0){
                       switch2.setChecked(true);
                       tvDelaytime.setText("时间"+list.get(i).getDelaytime()+"秒");
                   }else{
        ...
        switch2.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if(isChecked){
                    new DialogfourMin(ScenarioEditActivity.this, new DialogfourMin.OnChooseListener() {
                        @Override
                        public void onTimeCallback(int second) {
                            if(second >0){
                                if(sencedata != null){
                                    sencedata.setDelaytime((byte)(second));
                                }
                                tvDelaytime.setText("时间"+second+"秒");
                            }
        ...
        adapter.setOnDelayListener(this);
        ...
        arrow.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(layoutDelaytime.getVisibility() ==View.VISIBLE){
                    arrow.setImageResource(R.mipmap.arrow_down);
                    layoutDelaytime.setVisibility(View.GONE);
                }else{
                    arrow.setImageResource(R.mipmap.arrow_up);
                    layoutDelaytime.setVisibility(View.VISIBLE);
                }
            }
        });
...
//            menu.addMenuItem(openItem);
    ...
    private void showChooseDialog(){
        final String items[] = {"布防", "撤防"};
        if(sencedata == null){
            sencedata=new SenceData();
            sencedata.setuId(16449521);
            sencedata.setData2((byte) 2);
        }
        ...
        builder.setItems(items, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                switch (which){
                    case 0:
                        tvInstructions.setText("布防");
                        sencedata.setData1((byte) 1);
                        break;
                    case 1:
                        tvInstructions.setText("撤防");
                        sencedata.setData1((byte) 2);
                        break;
                }
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.tv_right_menu:
                ...
                if(list.size()>20){
                    showToast("单个场景中最多添加20个设备！");
                    return;
                }

                if(checkListrepeat(list)){
                    showToast("场景中不能有设置状态相同或冲突的设备！");
                    return;
                }
    ...
    private boolean checkListrepeat( List<SenceData> list){
        for (int i = 0; i < list.size()-1; i++) {
            for (int j = list.size()-1; j > i; j--) {
                if (list.get(j).getuId() == list.get(i).getuId()
                        && list.get(j).getDelaytime() ==  list.get(i).getDelaytime()) {
            ...
            if(list.size()==0){
                ...
                Subscription sub = Observable.create(new Observable.OnSubscribe<Integer>() {
                    ...
                    public void call(Subscriber<? super Integer> subscriber) {
                        ...
                        Serial mSerial = AppContext.getInstance().getSerialInstance();
                        mSerial.addDeviceToSence(sencename,0,(short)0,(byte)0,(byte)0,(byte)0,(byte)0,0,(byte)0,(byte)0);
                ...
                Observable.from(list).map(new Func1<SenceData, Integer>() {
                    public Integer call(SenceData info) {
                        AppContext.getInstance().getSerialInstance().addDeviceToSence(
                                sencename,info.getuId(),info.getDeviceId(),info.getData1(),info.getData2(),info.getData3(),info.getData4(),0,info.getDelaytime(), info.getFunid());
...
//                        }
//                        if(!tag) {
                          int ret = AppContext.getInstance().getSerialInstance().deleteSenceMember(sencename,info.getuId(),info.getFunid());
//                        }
                        LogUtil.e("Ret",String.valueOf(ret));
        ...
        switch (requestCode){
            case 1001:
                ...
                for (int i = 0; i <resultList.size() ; i++) {
                    switch (resultList.get(i).getDeviceId()) {
                        //色温灯
                        case DeviceList.DEVICE_ID_COLOR_TEMP1:
                        case DeviceList.DEVICE_ID_COLOR_TEMP2:
                            //彩灯
                        case DeviceList.DEVICE_ID_COLOR_PHILIPS:
                            resultList.get(i).setData2((byte)127);
                            break;
                    }
                    byte funid = getBigestfunid(resultList.get(i).getuId());
                    byte  mFunid = (byte) (funid+1);
                    resultList.get(i).setFunid(mFunid);
                }
    ...
    private byte getBigestfunid(int uid){
        byte max = 0;
        ...
        for (int i = 0; i <list.size() ; i++) {
            if(uid ==list.get(i).getuId() ){
                if(max<list.get(i).getFunid())
                max = list.get(i).getFunid();
            }
        }
        return max;
    }
    ...
    @Override
    public void onDelaytime(int position, int second) {
        list.get(position).setDelaytime((byte)(second));
    }
