...
import com.fbee.smarthome_wl.event.DefenseAreaIDsEvent;
...
//    }
     ...
     *  @param funID    同一个设备多次添加到场景时的区别标识
     */
    public native int addDeviceToSence(String senceName, int uid,
                                       short deviceId, byte data1, byte data2, byte data3, byte data4, int IRID, byte delaytime,byte funID);
     ...
     * @param sceneName 场景名
     * @param uId       设备uId
     * @param funid     同一设备在同一场景的区分标识
     */
    public native int deleteSenceMember(String sceneName, int uId,byte funid);
...
/***********************************************************************************/
     ...
     * @param funid     同一设备在同一场景的区分标识
     */
    public native int deleteSenceMember(String senceName, int uId, short irId,byte funid);
     ...
     * @return
     */
    public native int setGatedoorStateID2(DeviceInfo info, int state, byte[] gatedoorpassword);
    /**
     * 设置信道
     *
     * @param channel 信道
...
//		mContext.sendBroadcast(i);
     ...
     * @param attribID
     *            传感器数据类型：00 ： 温度 17：湿度
     * @param isDenfense 1：布防上报 0：撤防上报
     */
    public void arriveReport_CallBack(int uId, int data, char clusterId, char attribID,byte isDenfense) {
        LogUtil.e("arriveReport_CallBack","isDenfense=="+isDenfense+"----attribID==="+(short)attribID +"-------data=="+data+"-----uId=="+uId);
        ArriveReportCBInfo info = new ArriveReportCBInfo(uId, data, (short) clusterId, (short) attribID);
        info.setIsDenfense(isDenfense);
     ...
     * @param isDenfense 1：布防上报 0：撤防上报
     */
    public void arriveReportgatedoor_CallBack(int uId, byte[] data, char clusterId, char attribID,byte isDenfense) {
        LogUtil.e("arriveReportgatedoor_CallBack","isDenfense=="+isDenfense +"-------data=="+data+"-----uId=="+uId);
...
//        }
    ...
    public void arriveReportTransmit_CallBack(int uid, byte[] data) {
        LogUtil.e("arriveReportTransmit_CallBack",String.valueOf(uid));
    }
     ...
     * @param isdefense	1 布防/0 撤防
     */
    public void setZONEToAble_CallBack(byte zone, byte ret, byte isdefense) {
    ...
    public void getBindScene_CallBack(int uid, int sceneId) {
        RxBus.getInstance().post(new BindSenceInfoEvent(uid,sceneId));
    }
    ...
    public void getBindDevice_CallBack(int uid, int[] tarUid) {
        RxBus.getInstance().post(new BindDeviceInfoEvent(uid,tarUid));
    }
     ...
     *
     * @param zonetime  时区
     * @param auto  网关获取时间方式 0：手动设置 1：自动获取
     * @param year   年
     * @param hour  月
     * @param day   日
     * @param hour  时
     ...
     * @return
     */
    public void getGatewayTime_CallBack(int zonetime,int auto,int year,int month,int day,int hour,int minute,int second,int daylight) {

    }


    /**
     *  布防撤防 ：1：布防 联动有效  0：撤防  联动无效
     *
     *
     * @param IsArm
     * @return
     */
    public native int setZONEArm(byte IsArm);

    /***********************************************************************************/
    /**
     添加防区（新防区概念）
     @param name 防区名称
     @param enable 防区使能（布防/撤防）1：布防 0：撤防
     @discussion 客户端调用方法后 在"AddDefenseArea_CallBack(byte id,byte enable,String name，byte  success)"回调接收结果
     */
    public native  int createDefenseArea(byte[] name,byte enable);
...
/***********************************************************************************/
    /**
     删除防区（新防区概念）
     @param id 防区编号
     @discussion 客户端调用方法后 在"DeleteDefenseArea_CallBack(byte id,byte enable,String name，byte  success ) "回调接收结果
     */
    public  native int  destoryDefenseArea(byte id);

/***********************************************************************************/
    /**
     获取所有防区ID（新防区概念）
     @discussion 客户端调用方法后 在"GetAllDefenseAreaIDs_CallBack(byte id,byte enable，byte  success )"回调接收结果
     */
    public native int  getDefenseAreaInfos ();
...
/***********************************************************************************/
    ...
    /**
     获取防区详情（新防区概念）
     @param id 防区编号
     @discussion 客户端调用方法后 在"GetDefenseAreaDetail_CallBack(byte id ,byte enable,String  name, int[] devices)
     "回调接收结果
     */
    public native int getDefenseAreaDetails(byte id);
...
/***********************************************************************************/
    ...
    /**
     ...
     @param id 防区编号
     @param enable 防区使能（布防/撤防）1：布防 0：撤防
     @param password，）密码 布防需要
     @discussion 客户端调用方法后 在"EnableDefenseArea_CallBack(byte id,byte enable，byte success  ）
     "回调接收结果
     */

    public native  int setDefenseAreaToAble(byte id,byte enable,byte[] password);
...
/***********************************************************************************/
    ...
    /**
     添加设备到防区（新防区概念）
     @param id 防区编号
     @param devices 设备Uid 数组。
     @discussion 客户端调用方法后 在"AddeteDevicesFromDefenseArea_CallBack（byte id,int[] devices,byte success）
     "回调接收结果
     */
    public native int  addDeviceToDefenseArea( byte id ,int[] devices);
...
/***********************************************************************************/
    ...
    /**
     ...
     @param id 防区编号
     @param devices 设备Uid 数组。
     @discussion 客户端调用方法后 在"DeleteDevicesFromDefenseAre_CallBacke（byte id,int[] devices,byte success）
     "回调接收结果
     */
    public native int  delDeviceToDefenseArea(byte id ,int[] devices);
...
/***********************************************************************************/
    ...
    /**
     ...
     @param oldPsw 原始密码
     @param newPsw）新密码。
     @discussion 客户端调用方法后 在"ChangeDefenseAreaPassword_CallBacke（,byte success）
     "回调接收结果
     */
    public native int   changeDefenseAreaPassword(byte[] oldPsw ,byte[] newPsw);
...
/***********************************************************************************/
    /**
     ...
     @discussion 客户端调用方法后"createDefenseArea(byte[] name,byte enable)"后产生该回调
     */
    public void AddDefenseArea_CallBack(byte id,byte enable,String name,byte  success ){
        RxBus.getInstance().post(new AddDefenseAreaEvent(id,enable,name,success));
    }
...
/***********************************************************************************/
    ...
    /**
     ...
     @discussion 客户端调用方法 " destoryDefenseArea(byte id)"后产生该回调
     */
    public void DeleteDefenseArea_CallBack(byte id,byte success ){}
    ...
    /**
     ...
     @param enable 防区使能（布防/撤防）1：布防 0：撤防
     @param success 是否成功
     @discussion 客户端调用方法"setDefenseAreaToAble（byte id,byte enable,byte password）"后产生该回调
     */
    public void EnableDefenseArea_CallBack(byte id,byte enable,byte  success ){
        RxBus.getInstance().post(new EnableDefensAreaEvent(id,enable,success));
...
/***********************************************************************************/
    /**
     ...
     @param defenseAreaIDs 所有的防区编号
     @discussion 客户端调用方法"getDefenseAreaInfos（）"后产生该回调
     */

    public void  GetAllDefenseAreaIDs_CallBack(byte[] defenseAreaIDs ){
        RxBus.getInstance().post(new DefenseAreaIDsEvent(defenseAreaIDs));
    }
...
/***********************************************************************************/
    /**
     ...
     @param id 防区编号
     @param name 名称
     @param enable 防区使能（布防/撤防）
     @param devices 防区中的设备（设备的UID）
     @discussion 客户端调用方法"getDefenseAreaDetails(byte id)"后产生该回调
     */
    public void GetDefenseAreaDetail_CallBack(byte id ,byte enable,String  name, int[] devices){
        RxBus.getInstance().post(new DefenseAreaEvent(id,enable,name,devices));
...
/***********************************************************************************/
    /**
     ...
     @param devices 添加的设备
     @param success 是否添加成功
     @discussion 客户端调用方法"addDeviceToDefenseArea（byte id ,int[] devices）"后产生该回调
     */
    public void AddDevicesToDefenseArea_CallBack(byte id,int[] devices,byte success){
//        RxBus.getInstance().post(new AddDevicesToDefenseArea(id,devices,success));
    }
...
/***********************************************************************************/
    ...
    /**
     ...
     @param devices 添加的设备
     @param success 是否删除成功
     @discussion 客户端调用方法" delDeviceToDefenseArea（byte id ,int[] devices）"后产生该回调
     */
    public void  DeleteDevicesFromDefenseArea_CallBack(byte id,int[] devices,byte success){}

/***********************************************************************************/

    /**
     @param success 是否修改密码成功
     @discussion 客户端调用方法" changeDefenseAreaPassword（byte[] oldPsw ,byte[] newPsw）"后产生该回调
     */
    public void  ChangeDefenseAreaPassword_CallBack(byte success){}
