...
public class MainPresenter extends BaseCommonPresenter<MainContract.View> implements MainContract.Presenter {
    ...
    @Override
    public void requestToken(String type) {
        Ams.AMSBean.HeaderBean header = new Ams.AMSBean.HeaderBean();
        header.setApi_version("1.0");
        header.setMessage_type(type);
        header.setSeq_id(mSeqid.getAndIncrement()+"");
        Ams.AMSBean.BodyBean bodyBean=new Ams.AMSBean.BodyBean();
        bodyBean.setUsername(PreferencesUtils.getString(PreferencesUtils.LOCAL_USERNAME));
        bodyBean.setSecret_key( PreferencesUtils.getString(PreferencesUtils.SECRET_KEY));
        bodyBean.setVendor_name("ys7");
        Ams ams = new Ams();
        Ams.AMSBean amsbean=new Ams.AMSBean();
                ...
                if (!mCompositeSubscription.isUnsubscribed()) {
                    if (null != json) {
                        ...
                        if (null == jsonObj || jsonObj.size() == 0)
                            return;
                        AmsR res = new Gson().fromJson(json.toString(), AmsR.class);
                        if(res.getAMS().getHeader()!=null&&res.getAMS().getHeader().getHttp_code().equals("200")){
                            if(res.getAMS().getHeader().getReturn_string().equals("RETURN_SUCCESS_OK_STRING")){
                                EZOpenSDK.getInstance().setAccessToken(res.getAMS().getBody().getAccessToken());
                            }
        ...
        Subscription subscription = mApiWrapper.getToken(ams).subscribe(subscriber);
